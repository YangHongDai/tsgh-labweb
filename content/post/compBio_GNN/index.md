...
1. 通過 GNN 層進行`信息傳遞 (Message Passing)`: 嵌入後，GNN 在多層網絡中進行一系列的消息傳遞（message passing）步驟。這一過程類似於 CNN 中的卷積操作，CNN 是從鄰近像素中聚合信息，而 GNN 則是從相鄰節點中聚合信息。



2. 聚合與更新
在每一層 GNN 中，每個節點都從其鄰居處接收信息（通常稱為“消息”）。
節點將這些消息聚合起來，與自身的潛在向量結合後更新其狀態。這種聚合方法可以是求和（sum）、均值（mean）、或最大池化（max pooling），具體取決於任務需求。
1. 多層結構
信息通過多層傳遞，每層使節點可以“看到”圖中更遠的鄰居。例如，經過兩層後，每個節點可以融入來自其兩跳鄰居的信息。
1. 最終狀態與輸出生成
當圖經過多層 GNN 處理後，節點達到最終狀態，該狀態包含從周圍節點學到的信息。
這些最終狀態可以用於生成多種預測：
節點級任務：使用每個節點的最終狀態進行預測。
邊級任務：分析相鄰節點最終狀態之間的關係。
圖級任務：將所有節點的最終狀態池化成單一圖表示，用於分類或回歸。
...
以下是一個使用 GNN 進行分子預測的假設例子：

初始設置：
圖形化表示分子：每個原子是節點，特徵包括原子質量、電負性和雜化。
每條邊表示鍵，特徵包括鍵類型（單鍵、雙鍵、芳香鍵等）。
初始嵌入：
每個節點用一個潛在向量表示其初始特徵。
信息傳遞：
在多層 GNN 中，節點之間交換並聚合信息。
節點的潛在向量在多層信息傳遞過程中迭代更新。
預測：
使用節點的最終嵌入向量進行任務，例如預測分子與特定蛋白結合的可能性。
GNN 的操作類型

GNN 層中的操作旨在將卷積操作推廣到圖結構，其中空間結構由節點之間的邊定義，而不是固定的空間坐標。

1. 圖上的卷積（Convolution on Graphs）
在 CNN 中，卷積在固定的網格上進行；而在 GNN 中，卷積基於每個節點的鄰域進行。
這種局部特徵的聚合使得每個節點可以從其鄰居中學習，同時保留圖的固有結構。
2. 池化與讀出（Pooling and Readout）
與 CNN 中的池化層類似，GNN 的池化用於減少維度並聚合節點信息。
池化可以應用於子圖內的節點，也可以應用於整個圖，用於生成圖級輸出。
3. 正規化（Normalization）
由於節點的度數（連接數）可能不同，對聚合的消息進行正規化有助於防止模型過度依賴於高連接節點，並保證信息流的平衡。
GNN 在分子建模與藥物發現中的優勢

GNN 的結構化消息傳遞使其能夠有效地從複雜的關係中學習，這在分子和生物應用中尤為重要。主要優勢包括：

1. 靈活處理複雜結構
GNN 能夠處理分子圖的不規則和非歐幾里得結構，其中原子和鍵並不遵循網格狀結構。
2. 高效利用結構信息
通過嵌入和消息傳遞，GNN 能夠內在地尊重分子內的空間和化學關係。
3. 可擴展性
GNN 通過基於鄰域的聚合機制，可以有效處理大型圖形數據，例如虛擬篩選中的分子庫。
GNN 的消息傳遞機制（Message Passing）

消息傳遞是 GNN 的核心操作，使節點能夠從鄰居處接收和聚合信息，形成更豐富的上下文表示。

消息傳遞的步驟
1. 消息創建（Message Creation）

對於圖中的每個節點，GNN 從其鄰居創建消息。
這通常通過將每個鄰居節點的潛在向量（隱藏表示）乘以一個可學習的權重矩陣來完成。
2. 聚合（Aggregation）

聚合將所有鄰居的消息合併成一個向量，表示所有鄰居的綜合影響。
常用的聚合策略：
求和（Sum Aggregation）：簡單地將所有消息相加。
均值（Mean Aggregation）：計算消息的平均值，歸一化連接數。
最大池化（Max Pooling）：保留所有消息中最重要的元素。
3. 更新（Update Function）

節點將其當前的潛在向量與聚合的消息向量結合，更新自身狀態。
更新函數通常包括將節點的潛在向量乘以一個可學習的權重矩陣，並與聚合消息結合（例如相加或拼接）。
4. 多層傳遞

經過每層 GNN 後，節點的潛在向量更新，並輸入到下一層。
每增加一層，節點可以“看到”圖中更遠的節點，最終學習更長距離的依賴關係。